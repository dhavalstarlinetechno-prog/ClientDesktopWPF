// ============================================================
//  PDFBuilder.cs  –  WPF MVVM  |  .NET 8  |  iText7
// ============================================================
//  NuGet: dotnet add package itext7
//
//  ARCHITECTURE:
//  ┌──────────────────┐   PdfColumnAlignment   ┌─────────────────┐
//  │ InvoiceViewModel │ ──────────────────────► │   PdfService    │  implements IPdfService
//  └──────────────────┘                         └────────┬────────┘
//                                               converts │ TextAlignment
//                                                        ▼
//                                              ┌───────────────┐
//                                              │  PDFBuilder   │  internal iText7 helper
//                                              └───────────────┘  does NOT implement IPdfService
//
//  FIX LOG:
//  [FIX 1] Removed 'implements IPdfService' from PDFBuilder.
//          PDFBuilder is a PRIVATE iText7 helper. PdfService is the public adapter.
//          Before: PDFBuilder.AddGrid() expected PdfColumnAlignment → type mismatch
//                  because PdfService had already converted to TextAlignment.
//          After:  PDFBuilder.AddGrid() takes TextAlignment directly (iText7 native).
//
//  [FIX 2] Unknown PdfException — Root Cause: 'using var' on PdfWriter/PdfDocument/Document
//          causes DOUBLE-DISPOSE after doc.Close() → iText7 crash.
//          Fix: No 'using'. Added writer.SetCloseStream(false) so MemoryStream
//          stays open for ms.ToArray() after doc.Close().
//
//  [FIX 3] Metadata columns (RowType, IsHeader, IsTotal, Side, SecurityName)
//          were leaking into PDF as visible columns.
//          Fix: _skipColumns HashSet filters them before table is built.
//
//  [FIX 4] Row styling from RowType column:
//          SecurityHeader → yellow bg (#EFECC8) + bold
//          Total / SecurityTotal → light grey + bold
//          GrandTotal → dark grey + bold + larger font
//          SubTotal → light grey + normal weight
// ============================================================
...
// NOTE: 'Color', 'Table', 'Cell', 'Border', 'Paragraph', 'TextAlignment'
//       — ye sabhi iText7 ke hain. OpenXml.Spreadsheet using bilkul mat daalna.
...
namespace ClientDesktop.Infrastructure.Helpers
{
    ...
    public class PDFBuilder
    {
        ...
        // ── [FIX 3] Metadata columns that must NOT appear in PDF ────────────
        ...
        // ── MVVM-friendly events — no MessageBox inside ─────────────────────
        ...
        // ╔═══════════════════════════════════════════════════════════════════╗
        // ║                    FLUENT  PUBLIC  API                            ║
        // ║  (iText7 native types — called ONLY from PdfService)              ║
        // ╚═══════════════════════════════════════════════════════════════════╝
